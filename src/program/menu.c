...
Config loadConfig(const char *filename) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (sscanf(line, "flow_control_method: %d", &config.flow_control_method) == 1) continue;
        if (sscanf(line, "length: %d", &config.length) == 1) continue;
        if (sscanf(line, "queue_quantity: %d", &config.queue_quantity) == 1) continue;
        if (sscanf(line, "direction_change_period: %d", &config.direction_change_period) == 1) continue;
        if (sscanf(line, "w: %d", &config.w) == 1) continue;
        if (sscanf(line, "scheduling_method: %d", &config.scheduling_method) == 1) continue;
    }
...
void checkMissingValues(Config config) {
    if (!config.all_values_present) {
        ...
        if (config.flow_control_method == -1) printf("- flow_control_method is missing\n");
        if (config.length == -1) printf("- length is missing\n");
        if (config.queue_quantity == -1) printf("- queue_quantity is missing\n");
        if (config.direction_change_period == -1) printf("- direction_change_period is missing\n");
        if (config.w == -1) printf("- w is missing\n");
        if (config.scheduling_method == -1) printf("- scheduling_method is missing\n");
    } else {
...
void clearInputBuffer() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF); // Discard all characters until newline
}
